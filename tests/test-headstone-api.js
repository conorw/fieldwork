...
async function testHeadstoneAPI(imagePath, mimeType = "image/jpeg") {
  try {
    ...
    const response = await fetch(
      "http://localhost:3000/api/analyze-headstone",
      {
        method: "POST",
        headers: {
    ...
    result.persons.forEach((person, index) => {
      ...
      console.log(
        "Birth Location:",
        `${person.birth_city || ""} ${person.birth_country || ""}`.trim(),
      );
...
if (!imagePath) {
  console.log(
    "Usage: node test-headstone-api.js <path-to-image-file> [mime-type]",
  );
  console.log(
    "Example: node test-headstone-api.js ./sample-headstone.jpg image/jpeg",
  );
  process.exit(1);
}
