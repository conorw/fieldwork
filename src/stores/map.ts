...
export const useMapStore = defineStore("map", () => {
  ...
  const mapBounds = ref([
    ...
    [-6.2, 55.22], // Northeast - Ballycastle area
  ]);
  ...
  const finishExtentConfiguration = async (
    extent: [number, number, number, number],
  ): Promise<void> => {
    try {
      await locationsStore.updateLocation(locationsStore.selectedLocationId, {
        bbox: extent,
      });
      isConfiguringExtent.value = false;
    } catch (error) {
      console.error(
        "Error finishing extent configuration:",
        error,
    ...
    try {
      ...
      if (existingSettings2 && existingSettings2.length > 0) {
        ...
        try {
          ...
          console.warn(
            "Map extent deleted locally but server sync failed:",
            syncError,
          );
      ...
      console.error(
        "Error clearing map extent configuration:",
        error,
        JSON.stringify(error),
      );
      throw error;
  ...
  const getCurrentMapExtent = (): [number, number, number, number] | null => {
    ...
    try {
      ...
      console.error(
        "Error getting current map extent:",
        error,
        JSON.stringify(error),
      );
      return null;
  ...
  // Helper function to parse plot geometry (handles both GeoJSON and raw coordinates array)
  const parsePlotGeometry = (geometryString: string): number[][] | null => {
    try {
      ...
      if (Array.isArray(geometry)) {
        ...
        if (geometry.coordinates.length === 0 || !geometry.coordinates[0]) {
          console.warn("MapStore: Geometry coordinates array is empty");
          return null;
        }
        coordinates = geometry.coordinates[0];
      } else {
        console.warn("MapStore: Invalid geometry structure:", geometry);
        return null;
      }

      // Validate coordinates is an array
      if (!Array.isArray(coordinates) || coordinates.length === 0) {
        console.warn("MapStore: Invalid coordinates array");
        return null;
      }

      return coordinates;
    } catch (error) {
      console.error("MapStore: Error parsing geometry:", error);
      return null;
    }
  };
  ...
  const zoomToPlot = async (plot: any): Promise<void> => {
    ...
    try {
      // Parse geometry
      const coordinates = parsePlotGeometry(plot.geometry);
      if (!coordinates) {
        console.warn("Cannot zoom to plot: invalid geometry");
        return;
      }
      ...
      const isMapProjection = coordinates.some(
        (coord: number[]) =>
      ...
      if (isMapProjection) {
        ...
        mapCoordinates = coordinates.map((coord: number[]) =>
  ...
  const addPlotMarker = (plot: any): void => {
    if (!map.value || !plot.geometry) {
      console.warn(
        "Cannot add plot marker: map or plot geometry not available",
      );
      return;
    }
    ...
    try {
      ...
      if (source) {
        ...
        const existingPlot = existingFeatures.find(
          (feature: any) => feature.get("plot")?.id === plot.id,
        );
      ...
      const coordinates = parsePlotGeometry(plot.geometry);
      if (!coordinates) {
        console.warn("Cannot add plot marker: invalid geometry");
        return;
      }
  ...
  const removePlotMarker = (plotId: string): void => {
    ...
    try {
      ...
      const plotFeature = features.find(
        (feature: any) => feature.get("plot")?.id === plotId,
      );
  ...
  const enableInPlacePlotEditing = async (plot: any) => {
    ...
    if (!plotFeature) {
      console.error(
        "MapStore: Could not find plot feature for plot id:",
        plot.id,
      );
      console.error(
        "MapStore: Available plot features:",
        allFeatures.map((f: any) => f.get("plot")?.id),
      );
    ...
    const selectInteractions = interactions
      .getArray()
      .filter((interaction) => interaction.constructor.name === "Select");
    ...
    const { default: Transform } =
      (await import("ol-ext/interaction/Transform")) as any;
  ...
  const saveInPlacePlotCoordinates = async (
    ...
    try {
      ...
      console.error(
        "❌ MapStore: Error saving in-place plot coordinates:",
        error,
      );
      throw error;
    }
