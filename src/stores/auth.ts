...
export const useAuthStore = defineStore('auth', () => {
              ...
              try {
                ...
                if (!powerSyncStore.isInitialized) {
                  ...
                  powerSyncStore.initialize().catch((error: Error) => {
                    console.error('Failed to initialize PowerSync after sign in:', error)
                  })
                }
              } catch (error) {
    ...
    if (newSession) {
      try {
        ...
        if (!powerSyncStore.isInitialized) {
          ...
          powerSyncStore.initialize().catch((error: Error) => {
            console.error('Failed to initialize PowerSync after session set:', error)
          })
        }
      } catch (error) {
        console.error('Error importing PowerSync store:', error)
  ...
  const checkUserHasLocations = async (): Promise<boolean> => {
    ...
    try {
      ...
      console.warn('PowerSync not available for location check, falling back to Supabase:', error)
    }
    ...
    try {
      ...
      console.warn('Network error checking locations, assuming user has locations for offline mode:', error)
      return true
    }
  }
