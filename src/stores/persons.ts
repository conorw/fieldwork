...
export const usePersonsStore = defineStore("persons", () => {
  ...
  const loadPersonsByPlot = async (plotId: string) => {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "Failed to load persons for plot";
  ...
  const createPerson = async (
    ...
    const newPerson: PersonRecord = {
      ...
      age_at_death:
        personData.age_at_death !== null &&
        personData.age_at_death !== undefined
          ? personData.age_at_death.toString()
          : null,
    ...
    try {
      console.log(
        "🔍 PersonsStore: About to execute INSERT for person:",
        newPerson.id,
      );
      ...
      await powerSync.value.execute(
        ...
        [
          ...
          newPerson.title,
          newPerson.forename,
          newPerson.middle_name,
          newPerson.surname,
          newPerson.full_name,
          newPerson.address_line1,
          newPerson.address_line2,
          newPerson.town,
          newPerson.county,
          newPerson.country,
          newPerson.postcode,
          ...
          newPerson.race,
          newPerson.ethnicity,
          newPerson.created_by,
          newPerson.date_created,
          newPerson.last_updated_by,
          newPerson.last_updated_datetime,
          newPerson.birth_city,
          newPerson.birth_sub_country,
          newPerson.birth_country,
          newPerson.marital_status,
          newPerson.known_as,
          newPerson.maiden_name,
          newPerson.date_of_death,
          newPerson.age_at_death,
          newPerson.cause_of_death,
          newPerson.person_of_interest,
          newPerson.veteran,
      ...
      console.log(
        "✅ PersonsStore: INSERT executed successfully for person:",
        newPerson.id,
      );
  ...
  const updatePerson = async (
    ...
    if (
      ...
      if (person) {
        const title =
          updates.title !== undefined ? updates.title : person.title;
        const forename =
          updates.forename !== undefined ? updates.forename : person.forename;
        const middleName =
          updates.middle_name !== undefined
            ? updates.middle_name
            : person.middle_name;
        const surname =
          updates.surname !== undefined ? updates.surname : person.surname;

        fullName = [title, forename, middleName, surname]
          .filter(Boolean)
          .join(" ");
      }
    }
  ...
  const deletePerson = async (personId: string): Promise<boolean> => {
    ...
    try {
      await powerSync.value.execute("DELETE FROM persons WHERE id = ?", [
        personId,
      ]);
  ...
  const getPersonsByPlot = (plotId: string): PersonData[] => {
    ...
    console.log(
      "🔍 PersonsStore: getPersonsByPlot called for plot ID:",
      plotId,
    );
