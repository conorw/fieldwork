...
export const usePowerSyncStore = defineStore("powersync", () => {
  ...
  try {
    ...
    console.warn(
      "PowerSync: Continuing without database - app will work in offline mode",
    );
  }
  ...
  const initialize = async () => {
    ...
    if (isInitialized.value && powerSync.value) {
      console.log(
        "✅ [PowerSync] Already initialized, returning existing instance",
      );
    ...
    if (isConnecting.value && initializationPromise) {
      console.log(
        `⏳ [PowerSync] Initialization in progress (called from ${callStack}), waiting for existing initialization to complete...`,
      );
    ...
    if (!db) {
      console.warn(
        "PowerSync: Database instance is null, cannot initialize - continuing in offline mode",
      );
    ...
    console.log(
      `🚀 [PowerSync] Starting NEW initialization (called from ${callStack})`,
    );
    ...
    initializationPromise = (async () => {
      try {
        ...
        console.warn(
          "PowerSync initialization failed, continuing in offline mode",
        );
        return null;
      } finally {
        isConnecting.value = false;
  ...
  const createNewPlot = async (
    ...
    console.log(
      "PowerSync: Original geometry type:",
      typeof plotData.geometry,
      "isProxy:",
      plotData.geometry?.constructor?.name,
    );
    console.log(
      "PowerSync: Serialized geometry as JSON string:",
      serializedGeometry,
    );
    ...
    try {
      ...
      await powerSync.value.execute(
        ...
        [
          ...
          newPlot.number,
          newPlot.status,
          newPlot.location_id || null,
          newPlot.temp_plot_id || null,
          newPlot.date_created,
          newPlot.date_modified,
          newPlot.created_by,
          newPlot.modified_by,
          newPlot.notes || "",
        ],
      ...
      console.log(
        "✅ PowerSync: Plot inserted into local database successfully",
      );
      console.log(
        "🔄 PowerSync: Plot will be queued for sync to remote database",
      );
      ...
      const createdPlot = (await powerSync.value.get(
        "SELECT * FROM plots WHERE id = ?",
        [plotId],
      )) as PlotRecord;

      console.log(
        "✅ PowerSync: Retrieved created plot from local database:",
        createdPlot,
      );
  ...
  const updateExistingPlot = async (
    ...
    const existingPlot = (await powerSync.value.get(
      "SELECT * FROM plots WHERE id = ?",
      [id],
    )) as PlotRecord | null;
    if (!existingPlot) {
    ...
    await powerSync.value.execute(
      ...
      [
        updatedPlot.geometry,
        updatedPlot.section,
        updatedPlot.row,
        updatedPlot.number,
        updatedPlot.status,
        updatedPlot.location_id || null,
        updatedPlot.date_modified,
        updatedPlot.modified_by,
        updatedPlot.notes || "",
        id,
      ],
    );
    ...
    const result = (await powerSync.value.get(
      "SELECT * FROM plots WHERE id = ?",
      [id],
    )) as PlotRecord;
    return result;
  ...
  const getPlotsByLocation = async (
    locationId: string,
  ): Promise<PlotRecord[]> => {
    if (!powerSync.value) throw new Error("PowerSync not initialized");

    const plots = await powerSync.value.getAll(
      "SELECT * FROM plots WHERE location_id = ? ORDER BY date_created DESC",
      [locationId],
    );
    return plots as PlotRecord[];
  };

  const findPlotByTempId = async (
    tempPlotId: string,
  ): Promise<PlotRecord | null> => {
    if (!powerSync.value) throw new Error("PowerSync not initialized");
    try {
      const plot = (await powerSync.value.get(
        "SELECT * FROM plots WHERE temp_plot_id = ?",
        [tempPlotId],
      )) as PlotRecord;
      return plot;
    } catch (error) {
      ...
      if (
        ...
        error.message.includes("Result set is empty")
      ) {
        return null;
      }
      throw error;
  ...
  const addNewLocation = async (
    ...
    const newLocation = {
      ...
      bbox:
        typeof locationData.bbox === "string"
          ? locationData.bbox
          : JSON.stringify(locationData.bbox),
      min_zoom: locationData.min_zoom || "8",
      max_zoom: locationData.max_zoom || "18",
    ...
    await powerSync.value.execute(
      "INSERT INTO locations (id, name, bbox, min_zoom, max_zoom, pmtiles_url, date_created, date_modified, created_by, owner_id, is_public) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        newLocation.id,
        newLocation.name,
        newLocation.bbox,
        newLocation.min_zoom,
        newLocation.max_zoom,
        newLocation.pmtiles_url,
        newLocation.date_created,
        newLocation.date_modified,
        newLocation.created_by,
        newLocation.owner_id,
    ...
    // Get the last inserted location
    const createdLocation = (await powerSync.value.get(
  ...
  const addPersonImage = async (
    ...
    try {
      ...
      const { imageProcessingService } =
        await import("../utils/imageProcessingService");
      ...
      await powerSync.value.execute(
        ...
        [
          personImage.id,
          personImage.person_id,
          personImage.file_name,
          personImage.data,
          personImage.thumbnail_data,
          personImage.cloud_url,
          personImage.original_size,
          personImage.thumbnail_size,
          personImage.dimensions,
          personImage.format,
          personImage.date_created,
          personImage.created_by,
        ],
      );

      console.log(
        "✅ PowerSync: Hybrid person image inserted into local database successfully",
      );
      console.log(
        "🔄 PowerSync: Image will be queued for sync to remote database",
      );
      ...
      const createdImage = (await powerSync.value.get(
        "SELECT * FROM person_images WHERE id = ?",
        [imageId],
      )) as any;

      console.log(
        "✅ PowerSync: Retrieved created hybrid person image from local database:",
        {
          id: createdImage.id,
      ...
      console.log(
        "🔄 PowerSync: Hybrid person image is now ready for background sync",
      );

      return createdImage;
    } catch (error) {
      console.error(
        "❌ PowerSync: Error processing hybrid person image:",
        error,
      );
      ...
      console.log(
        "🔄 PowerSync: Attempting to create thumbnail only (no full-size storage)...",
      );
      ...
      try {
        ...
        const thumbnail =
          await imageProcessingService.createThumbnailFromBase64(
        ...
        await powerSync.value.execute(
          ...
          [
            fallbackImage.id,
            fallbackImage.person_id,
            fallbackImage.file_name,
            fallbackImage.data,
            fallbackImage.thumbnail_data,
            fallbackImage.cloud_url,
            fallbackImage.original_size,
            fallbackImage.thumbnail_size,
            fallbackImage.dimensions,
            fallbackImage.format,
            fallbackImage.date_created,
            fallbackImage.created_by,
          ],
        );

        console.log(
          "✅ PowerSync: Stored person thumbnail only (no full-size data)",
        );
        const createdImage = (await powerSync.value.get(
          "SELECT * FROM person_images WHERE id = ?",
          [imageId],
        ...
        console.error(
          "❌ PowerSync: Failed to create person thumbnail:",
          thumbnailError,
        );
        throw error;
      }
  ...
  const addPlotImage = async (
    ...
    console.log(
      "🔄 PowerSync: Headstone analysis requested:",
      options.analyzeForHeadstone,
    );
    ...
    try {
      ...
      await powerSync.value.execute(
        ...
        [
          ...
          plotImage.file_name,
          plotImage.data,
          plotImage.thumbnail_data,
          plotImage.cloud_url,
          plotImage.original_size,
          plotImage.thumbnail_size,
          plotImage.dimensions,
          plotImage.format,
          plotImage.date_created,
          plotImage.created_by,
        ],
      );

      console.log(
        "✅ PowerSync: Hybrid image inserted into local database successfully",
      );
      console.log(
        "🔄 PowerSync: Image will be queued for sync to remote database",
      );
      ...
      // Get the created image by ID
      const createdImage = (await powerSync.value.get(
        "SELECT * FROM plot_images WHERE id = ?",
        [imageId],
      )) as PlotImageRecord;

      console.log(
        "✅ PowerSync: Retrieved created hybrid image from local database:",
        {
          id: createdImage.id,
      ...
      console.log(
        "🔄 PowerSync: Hybrid image is now ready for background sync",
      );
      ...
      if (options.analyzeForHeadstone) {
        ...
        try {
          ...
          const analysisResult =
            await headstoneAnalysisService.analyzeHeadstoneImage(file, plotId);

          if (analysisResult.success) {
            console.log(
              `✅ PowerSync: Headstone analysis completed successfully - created ${analysisResult.persons.length} person(s)`,
            );

            // Emit event to notify UI about new persons
            window.dispatchEvent(
              new CustomEvent("headstone-analysis-completed", {
                detail: {
                  plotId,
            ...
            console.warn(
              "⚠️ PowerSync: Headstone analysis failed:",
              analysisResult.error,
            );

            // Emit event to notify UI about analysis failure
            window.dispatchEvent(
              new CustomEvent("headstone-analysis-failed", {
                detail: {
                  plotId,
                  imageId: createdImage.id,
          ...
          console.error(
            "❌ PowerSync: Error in headstone analysis:",
            analysisError,
          );

          // Emit event to notify UI about analysis error
          window.dispatchEvent(
            new CustomEvent("headstone-analysis-error", {
              detail: {
                plotId,
                imageId: createdImage.id,
                error:
      ...
      try {
        ...
        // If even thumbnail creation fails, throw the original error
        console.error(
          "❌ PowerSync: Failed to create thumbnail:",
          thumbnailError,
        );
        throw error;
      }
  ...
  const updatePlotGeometry = async (
    plotId: string,
    newGeometry: string,
  ): Promise<void> => {
  ...
  const deletePlot = async (plotId: string): Promise<void> => {
    ...
    try {
      ...
      await powerSyncInstance.execute("DELETE FROM plots WHERE id = ?", [
        plotId,
      ]);
  ...
  const generateNextPlotNumber = async (): Promise<string> => {
    ...
    if (!powerSyncInstance) {
      console.error(
        "PowerSync client not initialized in generateNextPlotNumber",
      );
...
export const useReactivePowerSyncQuery = (
  ...
  const execute = async () => {
    ...
    try {
      ...
      console.log(
        `📊 [PowerSync Query] Starting query: ${queryString.substring(0, 100)}...`,
      );
      console.log(
        `📊 [PowerSync Query] DB ready state: ${powerSyncStore.powerSync?.ready}, closed: ${powerSyncStore.powerSync?.closed}`,
      );
      ...
      if (querySetupTime > 10) {
        console.warn(
          `📊 [PowerSync Query] ⚠️ Query setup took ${querySetupTime.toFixed(2)}ms (unexpected overhead)`,
        );
      }
      ...
      const result = await powerSyncStore.powerSync.getAll(
        queryString,
        actualParams,
      );
      ...
      if (queryExecutionTime > 1000) {
        console.warn(
          `[PowerSync Query] Slow query: ${queryExecutionTime.toFixed(2)}ms - possible IndexedDB lock or sync conflict`,
        );
      }
  ...
  watch(
    [query, params, () => powerSyncStore.isInitialized],
    (
      [newQuery, newParams, initialized],
      [oldQuery, oldParams, wasInitialized],
    ) => {
      if (initialized) {
...
export const usePlots = () => {
  ...
  const startWatch = async () => {
    ...
    try {
      ...
      const query =
        "SELECT id, geometry, section, row, number, status, location_id, temp_plot_id, date_created, date_modified, created_by, modified_by, notes FROM plots WHERE location_id = ? ORDER BY date_created DESC";
  ...
  watch(
    [selectedLocationId, () => powerSyncStore.isInitialized],
    async ([newLocationId, initialized], [oldLocationId]) => {
  ...
  return {
    ...
    refetch: () => {
      console.log(
        "📊 [usePlots] Refetch called (watch handles updates automatically, restarting watch)",
      );
      startWatch();
    },
  };
...
export const usePlot = (id: string) => {
  ...
  const queryResult = usePowerSyncQuery("SELECT * FROM plots WHERE id = ?", [
    id,
  ]);
...
export const usePlotImages = (
  plotId: string | Ref<string> | ComputedRef<string>,
) => {
  const powerSyncStore = usePowerSyncStore();
  const data = ref<any[]>([]);
  const loading = ref(false);
  ...
  const startWatch = async () => {
    if (!powerSyncStore.powerSync) {
      console.log(
        "📊 [usePlotImages] PowerSync not initialized, skipping watch",
      );
      return;
    }
    ...
    try {
      console.log(
        `📊 [usePlotImages] Starting watch for plot: ${currentPlotId}`,
      );
      const queryString =
        "SELECT * FROM plot_images WHERE plot_id = ? ORDER BY date_created DESC";

      // Do an initial query to get data immediately
      const initialResult = await powerSyncStore.powerSync.getAll(queryString, [
        currentPlotId,
      ]);
      data.value = initialResult;
      loading.value = false;
      console.log(
        `📊 [usePlotImages] Initial query completed: ${initialResult.length} images`,
      );

      // Use PowerSync's watch() which automatically updates when plot_images table changes
      watchIterator = powerSyncStore.powerSync.watch(queryString, [
        currentPlotId,
      ]);
      ...
      (async () => {
        try {
          for await (const result of watchIterator!) {
            ...
            loading.value = false;
            console.log(
              `📊 [usePlotImages] Watch updated: ${rows.length} images`,
            );
          }
        } catch (err: any) {
          if (watchActive) {
  ...
  watch(
    [plotIdRef, () => powerSyncStore.isInitialized],
    async ([newPlotId, initialized], [oldPlotId]) => {
      if (initialized && powerSyncStore.powerSync) {
...
export const usePersonImages = (personId: string) => {
  return usePowerSyncQuery(
    "SELECT * FROM person_images WHERE person_id = ? ORDER BY date_created DESC",
    [personId],
  );
};
