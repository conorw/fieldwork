...
export const useSettingsStore = defineStore("settings", () => {
    ...
    try {
      ...
      if (savedSettings) {
        ...
        if (parsed.minZoom >= 16 || parsed.maxZoom >= 19) {
          console.log(
            "Migrating from old zoom levels to new PMTiles-compatible levels",
          );
  ...
  const startTileDownload = (extent: any, minZoom: number, maxZoom: number) => {
    ...
    localStorage.setItem(
      "tileDownloadState",
      JSON.stringify(tileDownloadState.value),
    );

    console.log("Tile download started:", tileDownloadState.value);
  };

  const updateTileDownloadProgress = (
    progress: number,
    ...
    if (tileDownloadState.value.startTime && progress > 0) {
      ...
      tileDownloadState.value.estimatedTimeRemaining = Math.max(
        0,
