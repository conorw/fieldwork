...
export class CapacitorCameraService {
  ...
  async requestPermissions(): Promise<PermissionStatus> {
    try {
      if (this.isNativePlatform()) {
        ...
        console.log(
          "CapacitorCamera: Native permissions granted:",
          permissions,
        );
        return permissions;
      } else {
  ...
  private async takePhotoWithCamera(resolve: Function, reject: Function) {
    try {
      console.log(
        "CapacitorCamera: Attempting to use camera with getUserMedia",
      );
      ...
      captureButton.onclick = () => {
        try {
          ...
          console.log(
            "CapacitorCamera: Photo captured successfully with camera",
          );
          resolve({
            dataUrl: dataUrl,
            format: "image/jpeg",
      ...
      console.error(
        "CapacitorCamera: Camera access failed, falling back to file picker:",
        error,
      );
  ...
  private pickFromGalleryWeb(): Promise<CameraResult> {
    return new Promise((resolve, reject) => {
      try {
        console.log(
          "CapacitorCamera: Opening file picker for gallery selection",
        );
  ...
  async takeGravePhoto(): Promise<CameraResult> {
    try {
      ...
      if (this.isNativePlatform()) {
        ...
        console.log(
          "CapacitorCamera: Native grave photo captured successfully",
        );
        return {
  ...
  async takePhotoWithChoice(): Promise<CameraResult> {
    try {
      ...
      if (this.isNativePlatform()) {
        ...
        // Use web camera API fallback
        console.log(
          "CapacitorCamera: Using web camera fallback for user choice",
        );
        const result = await this.takePhotoWeb();
  ...
  async takeQuickPhoto(): Promise<CameraResult> {
    try {
      ...
      if (this.isNativePlatform()) {
        ...
        // Use web camera API fallback
        console.log(
          "CapacitorCamera: Using web camera fallback for quick photo",
        );
        const result = await this.takePhotoWeb();
  ...
  async pickFromGallery(): Promise<CameraResult> {
    try {
      ...
      if (this.isNativePlatform()) {
        ...
        console.log(
          "CapacitorCamera: Using web file picker fallback for gallery",
        );
  ...
  dataUrlToBlob(dataUrl: string): Blob {
    try {
      ...
      console.error(
        "CapacitorCamera: Error converting data URL to blob:",
        error,
      );
