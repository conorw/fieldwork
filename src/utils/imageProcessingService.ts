...
class ImageProcessingService {
  ...
  async processImage(file: File): Promise<ImageProcessingResult> {
    console.log(
      "ImageProcessingService: Processing image:",
      file.name,
      file.size,
      "bytes",
    );
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "ImageProcessingService: Cloud upload failed:",
          uploadError,
        );
      }
      ...
      console.log("ImageProcessingService: Processing complete:", {
        ...
        compressionRatio: Math.round(
          (1 - result.metadata.thumbnailSize / result.metadata.originalSize) *
            100,
        ),
      });
      ...
      img.onload = () => {
        ...
        canvas.toBlob(
          (blob) => {
            ...
            console.log(
              `ImageProcessingService: Compressed image from ${file.size} to ${compressedFile.size} bytes`,
            );
  ...
  private async uploadToVercelStorage(file: File): Promise<string> {
    try {
      ...
      console.log(
        "ImageProcessingService: Uploading to Vercel storage:",
        fileName,
      );
      ...
      console.log(
        "ImageProcessingService: Base64 conversion successful, length:",
        fileData.length,
      );
      ...
      if (!response.ok) {
        ...
        console.error(
          "ImageProcessingService: Upload response error:",
          response.status,
          errorText,
        );
  ...
  async createThumbnailFromBase64(base64Data: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const thumbnailBase64 = canvas
          .toDataURL("image/jpeg", this.THUMBNAIL_QUALITY)
          .split(",")[1];
