...
class ImageMigrationService {
  ...
  async migrateImages(
    ...
    try {
      ...
      console.log(
        `🔄 ImageMigration: Found ${imagesToMigrate.length} images to migrate`,
      );
      ...
      console.log(
        `📊 ImageMigration: Results - ${this.currentProgress.successful} successful, ${this.currentProgress.failed} failed`,
      );
  ...
  private async processBatch(
    ...
    const promises = batch.map((image) =>
      this.processImage(image, powerSync, skipExisting, options),
    );
  ...
  private async processImage(
    ...
    try {
      ...
      if (skipExisting && image.thumbnail_data && image.cloud_url) {
        console.log(
          `⏭️ ImageMigration: Skipping already migrated image: ${image.file_name}`,
        );
      ...
      console.log(
        `🔄 ImageMigration: Processing image: ${image.file_name} (${image.data.length} chars)`,
      );
      ...
      console.log(
        `✅ ImageMigration: Successfully migrated image: ${image.file_name}`,
      );
      this.currentProgress.successful++;
    } catch (error) {
      console.error(
        `❌ ImageMigration: Failed to migrate image ${image.file_name}:`,
        error,
      );
  ...
  async getImageStats(powerSync: any): Promise<{
    ...
    for (const image of allImages) {
      ...
      const hasThumbnail =
        image.thumbnail_data && image.thumbnail_data.length > 0;
      ...
      if (hasData && hasThumbnail && hasPreview && hasCloudUrl) {
        ...
        const originalSize =
          parseInt(image.original_size) || image.data.length * 0.75;
