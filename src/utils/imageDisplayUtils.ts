...
export function getImageSource(
  ...
  if (state.loading) {
    ...
    if (image.originalData) {
      return image.originalData.startsWith("data:")
        ? image.originalData
        : `data:image/jpeg;base64,${image.originalData}`;
    }
  }
  ...
  switch (state.loadedLevel) {
    ...
    case "legacy":
      return image.originalData
        ? image.originalData.startsWith("data:")
          ? image.originalData
          : `data:image/jpeg;base64,${image.originalData}`
        : "";
    default:
...
export function getCompressionRatio(image: ImageDisplayData): string {
  ...
  const ratio = Math.round(
    (1 - totalCompressedSize / image.originalSize) * 100,
  );
...
export class ProgressiveImageLoader {
  ...
  async loadImage(
    image: ImageDisplayData,
    targetLevel: "preview" | "full" = "preview",
  ): Promise<ImageDisplayState> {
    ...
    try {
      ...
      if (image.originalData && state.loadedLevel !== "legacy") {
        state.currentSrc = image.originalData.startsWith("data:")
          ? image.originalData
          : `data:image/jpeg;base64,${image.originalData}`;
