...
export class HeadstoneAnalysisService {
  ...
  private async storeAnalysisResultsForLater(
    tempPlotId: string,
    analysisData: any,
    ...
    try {
      ...
      const existingResults = JSON.parse(
        localStorage.getItem("pendingAnalysisResults") || "[]",
      );
      ...
      console.log(
        `HeadstoneAnalysisService: Stored analysis results for temp plot ID: ${tempPlotId}`,
      );
    } catch (error) {
  ...
  public async processStoredResultsForPlot(plotId: string, tempPlotId: string) {
    try {
      ...
      const resultIndex = storedResults.findIndex(
        (result: any) => result.tempPlotId === tempPlotId,
      );

      if (resultIndex !== -1) {
        const result = storedResults[resultIndex];
        console.log(
          `HeadstoneAnalysisService: Processing stored results for plot ${plotId}`,
        );
        ...
        if (result.personData && result.personData.length > 0) {
          for (const person of result.personData) {
            try {
              ...
              if (createdPerson) {
                console.log(
                  `HeadstoneAnalysisService: Created person ${createdPerson.full_name}`,
                );
              }
            } catch (error) {
        ...
        console.log(
          `HeadstoneAnalysisService: Completed processing stored results for plot ${plotId}`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (analysisMode === "local") {
        ...
        try {
          ...
          console.log(
            "HeadstoneAnalysisService: Local analysis completed:",
            analysisData,
          );
        } catch (localError) {
          const errorMsg =
          ...
          console.error(
            "HeadstoneAnalysisService: Local analysis failed:",
            localError,
          );
      ...
      const personData: PersonData[] = analysisData.persons.map((person: any) =>
        this.convertToPersonData(person, tempPlotId),
      );

      // if the plot already exists, update the plot notes and create persons
      if (personData.length > 0) {
        console.log(
          `HeadstoneAnalysisService: Processing analysis results for temp plot ID: ${tempPlotId}`,
        );
        ...
        if (plot) {
          console.log(
            `HeadstoneAnalysisService: Found plot ${plot.id}, updating notes and creating persons...`,
          );
          ...
          if (personData.length > 0) {
            console.log(
              `HeadstoneAnalysisService: Creating ${personData.length} person(s) automatically...`,
            );
          ...
          console.log(
            `HeadstoneAnalysisService: Plot not found for temp ID ${tempPlotId}, storing results for later processing...`,
          );
          // Store the analysis results for when the plot is created
          await this.storeAnalysisResultsForLater(
            tempPlotId,
            analysisData,
            personData,
          );
      ...
      if (
        ...
        console.log(
          "HeadstoneAnalysisService: OpenAI quota exceeded, skipping analysis silently",
        );
      } else if (
        ...
        console.log(
          "HeadstoneAnalysisService: OpenAI API key issue, skipping analysis silently",
        );
      }
  ...
  private convertToPersonData(apiPerson: any, plotId: string): PersonData {
    ...
    const fullName = [apiPerson.forename || "", apiPerson.surname || ""]
      .filter(Boolean)
      .join(" ");
  ...
  private async analyzeWithOpenAI(
    ...
    console.log(
      "HeadstoneAnalysisService: Making API request to:",
      this.API_ENDPOINT,
    );
    ...
    try {
      ...
      console.log(
        "HeadstoneAnalysisService: Analysis completed:",
        analysisData,
      );
    } catch (jsonError) {
      ...
      console.error(
        "HeadstoneAnalysisService: Failed to parse JSON response:",
        responseText,
      );
      throw new Error(
   ...
   * This reduces upload time and API processing time
   */
  private async compressImageForAPI(
    file: File,
