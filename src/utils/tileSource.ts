...
const ONLINE_PMTILES_URL =
  "https://6ftgthlf9zt2ofxl.public.blob.vercel-storage.com/pmtiles/OUTPUT.pmtiles";
...
export const createOfflinePMTilesTileSource = (
  sourceData: ArrayBuffer,
  sourceName = "Unknown",
) => {
  console.log(
    `${sourceName}: Creating offline PMTiles tile source from ArrayBuffer`,
  );
  ...
  try {
    ...
    console.log(
      `${sourceName}: Offline PMTiles tile source created successfully`,
    );
    return vectorLayer;
  } catch (error) {
    console.error(
      `${sourceName}: Error creating offline PMTiles tile source:`,
      error,
    );
...
export const createBestTileSource = async (
  location: PMTilesLocation | null | undefined,
  sourceName = "Unknown",
) => {
  console.log(`${sourceName}: createBestTileSource called with location:`, {
    id: location?.id,
    name: location?.name,
