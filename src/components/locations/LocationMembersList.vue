    ...
    <template #content>
      ...
      <div v-else class="space-y-2">
        ...
        >
          <div>
            <p class="font-medium">
              {{ member.user_email || member.user_id || "User" }}
            </p>
            <p
              v-if="member.user_email && member.user_email !== member.user_id"
...
const canManageMembers = computed(
  () => isOwner.value || props.userRole === "admin",
);
...
const loadMembers = async () => {
  ...
  try {
        ...
        try {
          ...
          if (supabaseMembers) {
            const emailMap = new Map(
              supabaseMembers.map((m: any) => [m.user_id, m.user_email]),
            );
    ...
    const isOwner =
      location &&
      authStore.user &&
      (location as any).owner_id === authStore.user.id;
    ...
    if (
      ...
      results &&
      results.length === 1 &&
      (results[0] as any).user_id === authStore.user?.id
    ) {
      console.log(
        "Only seeing own member record, falling back to Supabase to get all members...",
      );
      try {
        const supabase = createSupabaseClient();
        ...
        if (supabaseError) {
          ...
          console.log(
            `Found ${supabaseMembers.length} members in Supabase vs ${results.length} in PowerSync`,
          );
