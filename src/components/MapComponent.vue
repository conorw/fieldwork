...
const createMap = async () => {
  ...
  try {
    ...
    const findSelectedLocation = async () => {
      try {
        // Ensure locations are loaded - wait if currently loading
        if (locationsStore.isLoading) {
          console.log("🗺️ [MapComponent] Locations are loading, waiting...");
        ...
        console.log("🗺️ [MapComponent] findSelectedLocation result:", {
          found: !!location,
          id: location?.id,
          name: location?.name,
        ...
        if (!location) {
          console.warn("🗺️ [MapComponent] No selected location found");
          return null;
        }
    ...
    // Ensure locations are loaded before creating map
    if (locationsStore.isLoading) {
      console.log("🗺️ [MapComponent] Locations are loading, waiting...");
      // Wait for loading to complete (max 10 seconds)
      let waitCount = 0;
    ...
    console.log("🗺️ [MapComponent] Selected location from store:", {
      id: selectedLocation?.id,
      name: selectedLocation?.name,
      pmtilesUrl: selectedLocation?.pmtilesUrl,
      hasPmtilesUrl: !!selectedLocation?.pmtilesUrl,
      pmtilesUrlType: typeof selectedLocation?.pmtilesUrl,
      pmtilesUrlLength: selectedLocation?.pmtilesUrl?.length,
      fullLocationObject: selectedLocation,
      locationsCount: locationsStore.locations.length,
      isLoading: locationsStore.isLoading,
    });

    if (selectedLocation) {
      const locationForTileSource = {
        id: selectedLocation.id,
        name: selectedLocation.name,
        bbox: selectedLocation.bbox,
        minZoom: selectedLocation.minZoom,
        ...
        if (
          ...
          if (props.extent && props.restrictExtent) {
            if (!isLocationWithinBounds(props.location, props.extent)) {
              showBoundsWarningOnce(
                "Your current location is outside the allowed map area.",
              );
            }
          }
    ...
    map.value.on("click", async (event) => {
      ...
      if (contextMenuVisible.value) {
        ...
        const isTouchEvent =
          event.originalEvent && event.originalEvent.type === "touchend";
        ...
        longPressTimer = setTimeout(async () => {
          try {
            ...
            if (!touchMoved) {
              ...
              const pixel = map.value.getPixelFromCoordinate(
                map.value.getCoordinateFromPixel([
                  touchStartPosition.x -
                    mapElement.value.getBoundingClientRect().left,
                  touchStartPosition.y -
                    mapElement.value.getBoundingClientRect().top,
                ]),
              );
      ...
      (event) => {
        ...
        if (touchStartPosition && longPressTimer) {
          const deltaX = Math.abs(
            event.touches[0].clientX - touchStartPosition.x,
          );
          const deltaY = Math.abs(
            event.touches[0].clientY - touchStartPosition.y,
          );
  ...
  try {
    ...
    if (currentLocation.value && props.extent && props.restrictExtent) {
      if (!isLocationWithinBounds(currentLocation.value, props.extent)) {
        showBoundsWarningOnce(
          "Your current location is outside the allowed map area. The map will show the configured area instead.",
        );
      }
    }
    ...
    showError(
      "Failed to initialize location tracking. Please check your location permissions.",
    );
  }
};
...
const autoCenterOnUserLocation = async () => {
  try {
    ...
    console.error(
      "MapComponent: Error auto-centering on user location:",
      error,
    );
  }
};
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.error(
          "MapComponent: Error requesting motion permission:",
          error,
        );
        userDirection.value = 0; // Default to North
      }
    ...
    console.error(
      "MapComponent: Error setting up orientation tracking:",
      error,
    );
    userDirection.value = 0; // Default to North
  }
...
watch(
  ...
  (
    [newCenter, newZoom, newMaxZoom, newMinZoom, newExtent, newRestrictExtent],
    [oldCenter, oldZoom, oldMaxZoom, oldMinZoom, oldExtent, oldRestrictExtent],
  ) => {
    if (!map.value) return;
    ...
    if (
      newRestrictExtent !== oldRestrictExtent ||
      (newExtent &&
        oldExtent &&
      ...
      if (newExtent && newRestrictExtent) {
        ...
        if (props.isLatLng) {
          viewExtent = fromLonLat(newExtent.slice(0, 2)).concat(
            fromLonLat(newExtent.slice(2, 4)),
          );
        }
  ...
  setViewToExtent: async (bbox, minZoom, maxZoom) => {
    if (map.value) {
      ...
      const currentZoom = Math.max(
        minZoom,
        Math.min(maxZoom, view.getZoom() || 16),
      );
