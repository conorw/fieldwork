<template>
  <div class="plot-editor h-full flex flex-col">
    ...
    <div
      v-if="!isPhotoMode"
      class="bg-primary-600 text-white px-4 py-3 rounded-t-lg"
    >
    ...
    <div :class="isPhotoMode ? 'p-0 flex flex-col h-full min-h-0' : 'p-4'">
      ...
      <div v-if="!isPhotoMode" class="bg-gray-100 rounded-lg p-4 mb-4">
        ...
        <ul class="text-sm text-gray-600 space-y-1">
          <li>
            • <strong>Click on the blue polygon</strong> to select it for
            editing
          </li>
          <li>
            • <strong>Drag the polygon</strong> to move it to the correct
            location
          </li>
          <li>
            • <strong>Drag the rotation handle</strong> (circular arrow) to
            rotate the plot
          </li>
          <li v-if="!allowCustomSizing">
            • <strong>Plot size is fixed</strong> at
            {{ selectedPlotSize?.width || 8 }}×{{
              selectedPlotSize?.height || 4
            }}ft and cannot be changed
          </li>
          <li v-else>
            • <strong>Drag the resize handles</strong> to change the plot size
          </li>
        </ul>
      </div>
      ...
      <div
        :class="
          isPhotoMode
            ? 'flex-1 bg-gray-200 rounded-lg relative overflow-hidden min-h-0'
            : 'h-full bg-gray-200 rounded-lg relative overflow-hidden mb-4'
        "
        style="height: 450px"
      >
        <!-- Loading State -->
      ...
      <div v-if="showPlotInfo" class="bg-gray-50 rounded-lg p-4 mb-4">
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div><span class="font-medium">Size:</span> {{ plotInfo.size }}</div>
          <div>
          ...
          <div><span class="font-medium">Date:</span> {{ plotInfo.date }}</div>
        </div>
      </div>
    </div>
  </div>
...
const createPolygonWithLocation = async (location) => {
  ...
  const halfShortSideDegrees =
    (heightMeters / 2 / earthRadius) * (180 / Math.PI); // East-West (longitude)

  // Adjust longitude offset for latitude (longitude lines get closer at higher latitudes)
  const halfShortSideLonDegrees =
    halfShortSideDegrees / Math.cos((centerLat * Math.PI) / 180);

  // Store the original direction
  const originalDirection =
    userDirection.value !== null ? userDirection.value : 0;
  ...
  if (originalDirection !== 0) {
    ...
    rotatedCorners = corners.map(([lon, lat]) => {
      ...
      // Apply rotation
      const rotatedX =
        translatedX * Math.cos(directionRad) -
        translatedY * Math.sin(directionRad);
      const rotatedY =
        translatedX * Math.sin(directionRad) +
        translatedY * Math.cos(directionRad);
  ...
  const shiftLon =
    (shiftDistance * Math.sin(originalDirectionRad)) /
...
const addExistingPlotToMap = async () => {
  ...
  try {
    ...
    if (!props.existingPlot.geometry) {
      throw new Error("Plot geometry is missing");
    }

    // Parse existing plot geometry
    let geometry = JSON.parse(props.existingPlot.geometry);

    // Handle both GeoJSON format and raw coordinates array format
    let coordinates;
    if (Array.isArray(geometry)) {
      // Raw coordinates array format: [[x, y], [x, y], ...]
      ...
      throw new Error(
        "Invalid geometry structure: must be GeoJSON object or coordinates array",
      );
    }
...
const addEditingInteractions = async (polygonFeature) => {
  try {
    ...
    try {
      ...
      console.error(
        "MapEdit: Failed to setup Transform interactions:",
        transformError,
      );
    }
  } catch (error) {
...
const addExistingPlotsToMap = async () => {
  ...
  try {
    ...
    for (const plot of props.existingPlots) {
      ...
      try {
        // Parse geometry
        let geometry = JSON.parse(plot.geometry);
        ...
        if (Array.isArray(geometry)) {
          ...
          if (geometry.coordinates.length === 0 || !geometry.coordinates[0]) {
            console.warn(
              "MapEdit: Geometry coordinates array is empty for plot:",
              plot.id,
            );
            continue;
          }
          ...
          console.warn(
            "MapEdit: Invalid geometry structure for plot:",
            plot.id,
            geometry,
          );
          continue;
        ...
        // Validate coordinates is an array
        if (!Array.isArray(coordinates) || coordinates.length === 0) {
          console.warn("MapEdit: Invalid coordinates array for plot:", plot.id);
          continue;
        }
        ...
        console.error(
          "MapEdit: Error processing plot geometry:",
          plot.id,
          error,
        );
      }
  ...
  (isVisible) => {
    if (isVisible && !map.value && !isInitializing.value) {
      nextTick(() => {
        initializeMap().catch((error) => {
          console.error(
            "MapEdit: Failed to initialize map after visibility change:",
            error,
          );
          emit("error", { message: "Failed to initialize map", error });
        });
  ...
  async (newDirection, oldDirection) => {
    ...
    if (
      ...
      try {
        ...
        const userLocationPoint = fromLonLat([
        ...
        const halfShortSideDegrees =
          (heightMeters / 2 / earthRadius) * (180 / Math.PI); // Short side (width) - perpendicular
        const halfLongSideDegrees =
          (widthMeters / 2 / earthRadius) * (180 / Math.PI); // Long side (length) - in facing direction
        ...
        // Adjust longitude offset for latitude (longitude lines get closer at higher latitudes)
        const halfShortSideLonDegrees =
          halfShortSideDegrees / Math.cos((userLat * Math.PI) / 180);
        ...
        const shiftToCenterLon =
          (shiftToCenter * Math.sin(originalDirectionRad)) /
  ...
  async (newLocation, oldLocation) => {
    ...
    if (
      ...
      try {
        ...
        const layers = map.value.getLayers();
        const vectorLayers = layers.getArray().filter(
          (layer) =>
            layer.getSource &&
            ...
            layer
              .getSource()
              .getFeatures()
              .some((f) => f === currentEditingFeature.value),
        );
        ...
        console.error(
          "❌ MapEdit: Error redrawing polygon with new location:",
          error,
        );
      }
    }
...
watch(
  () => props.selectedPlotSize,
  async (newPlotData, oldPlotData) => {
    if (
      newPlotData &&
      (!oldPlotData ||
        newPlotData.width !== oldPlotData.width ||
        newPlotData.height !== oldPlotData.height)
    ) {
...
watch(
  [effectiveMaxZoom, effectiveMinZoom, effectiveZoom],
  (newValues, oldValues) => {
