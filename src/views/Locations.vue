<template>
  <div class="h-full bg-surface-50">
      ...
      <div class="space-y-4">
        ...
        <Card>
          ...
          <template #content>
            <InputText
              ...
              :class="{ 'p-invalid': locationNameError }"
              class="w-full"
            />
            <InlineMessage
              v-if="locationNameError"
              severity="error"
              :closable="false"
            >
              {{ locationNameError }}
          ...
          <template #content>
            <div class="overflow-hidden" style="height: 50vh">
              <MapComponent
                ...
                view-name="ExtentConfiguration"
                tile-source-name="ExtentConfiguration"
        ...
        <Card>
          <template #content>
            <div class="space-y-3">
              <Button
                @click="resetRectangleToDefault"
                :disabled="isLoading || isSaving"
                label="Reset Area Size"
                severity="secondary"
                class="w-full"
              />

              <Button
                @click="handleSetExtent"
                :disabled="isSaving"
    ...
    <Dialog
      v-model:visible="showSaveLocationModal"
      modal
    ...
    >
      <div class="space-y-4">
        ...
        <div>
          ...
          <label for="public" class="ml-2 text-sm text-surface-700"
            >Make this location public</label
          >
        </div>
...
const createDefaultRectangle = async () => {
  try {
    ...
    const projectedCoords = rectangleCoords[0].map((coord) =>
      fromLonLat(coord),
    );
    ...
    console.log(
      ...
      vectorSource.value.getFeatures().length,
    );
  ...
  try {
    ...
    modifyInteraction.on("modifyend", async (event) => {
      ...
      if (geometry.getType() !== "Polygon") {
        console.error(
          "Geometry is not a polygon after modification:",
          geometry.getType(),
        );
        return;
      }
      ...
      const currentWidth = calculateDistance(
        centerLat,
        sw[0],
        centerLat,
        ne[0],
      );
      const currentHeight = calculateDistance(
        sw[1],
        centerLon,
        ne[1],
        centerLon,
      ...
      if (isTooLarge) {
        ...
        // Convert to map projection coordinates
        const projectedCoords = constrainedCoords[0].map((coord) =>
          fromLonLat(coord),
        );
        ...
        const constrainedDimensions =
          await calculatePolygonDimensions(constrainedExtent);
        ...
        const rectangularDimensions =
          await calculatePolygonDimensions(rectangularExtent);
    ...
    if (testFeature) {
      ...
      console.log(
        "Test feature extent:",
        testFeature.getGeometry().getExtent(),
      );
    } else {
...
const addRectangleInteractions = async (feature, source) => {
  try {
    ...
    const defaultModifyStyle = new Modify({ source: source })
      .getOverlay()
      .getStyleFunction();
    ...
    translateInteraction.on("translateend", async (event) => {
      ...
      if (geometry.getType() !== "Polygon") {
        console.error(
          "Geometry is not a polygon after translation:",
          geometry.getType(),
        );
        return;
      }
...
const handleSetExtent = async () => {
  ...
  if (!isPolygonValid.value) {
    alert(
      "Please draw a valid rectangle (max 1km x 1km) before setting the extent",
    );
    return;
  }
