<template>
  <div class="h-full overflow-y-auto">
    <div class="space-y-6 p-4">
          ...
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-xl font-semibold text-surface-900">
                Plot {{ effectivePlot?.section }}-{{ effectivePlot?.row }}-{{
                  effectivePlot?.number
                }}
              </h1>
              <p class="text-sm text-surface-600">
                Created
                {{
                  effectivePlot ? formatDate(effectivePlot.date_created) : ""
                }}
              </p>
            </div>
            <div class="flex items-center space-x-4">
              <Chip
                v-if="plotSyncStatus === 'pending'"
                label="Pending Sync"
                severity="warning"
                size="small"
              />
            ...
            <template #content>
              <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <Chip
                    v-else
                    :label="effectivePlot.status"
                    :severity="getStatusSeverity(effectivePlot.status)"
                  />
                </div>
              </div>
              ...
              <div class="mt-6">
                ...
                <Textarea
                  v-if="isEditing"
                  v-model="editablePlot.notes"
                  rows="3"
                  placeholder="Enter notes..."
                  class="w-full"
                />
                <p v-else class="text-sm">
                  {{ effectivePlot.notes || "No notes" }}
                </p>
              </div>
            </template>
          </Card>
                    ...
                    <div class="flex items-start justify-between">
                      <div class="flex-1">
                        <div class="flex items-center space-x-3 mb-3">
                          ...
                          />
                          <Chip
                            v-if="person.person_of_interest"
                        ...
                        >
                          ...
                          <div
                            ...
                            "
                          >
                            <span class="font-medium">Age at Death:</span>
                            {{ getAgeAtDeathDisplay(person) }}
                          </div>
                          ...
                          <div v-if="person.known_as">
                            <span class="font-medium">Known As:</span>
                            {{ person.known_as }}
                          </div>
                        </div>
      ...
      <!-- Person Modal -->
      <div
        v-if="showAddPersonModal || showEditPersonModal"
        class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50"
      >
        <div
          class="relative top-20 mx-auto p-8 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white"
        >
          <div class="mt-3">
            ...
            >
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                  <input
                    v-model="personForm.maiden_name"
                    type="text"
                    class="mt-1 input-field"
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  >
                  <select
                    v-model="personForm.marital_status"
                    class="mt-1 input-field"
                  >
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  >
                  <input
                    v-model="personForm.date_of_death"
                    type="date"
                    class="mt-1 input-field"
                  />
                </div>
                <div>
                  ...
                  >
                  <input
                    v-model.number="personForm.age_at_death"
                    type="number"
                    class="mt-1 input-field"
                    placeholder="Age"
                  />
                ...
                <div>
                  ...
                  >
                  <input
                    v-model="personForm.time_of_death"
                    type="time"
                    class="mt-1 input-field"
                  />
                </div>
              ...
              <div>
                ...
                >
                <input
                  v-model="personForm.cause_of_death"
                  type="text"
                  class="mt-1 input-field"
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  >
                  <input
                    v-model="personForm.birth_country"
                    type="text"
                    class="mt-1 input-field"
                    placeholder="Country"
                  />
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700"
                    >Birth Sub-Country</label
                  >
                  <input
                    v-model="personForm.birth_sub_country"
                    type="text"
                    class="mt-1 input-field"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  >
                  <input
                    v-model="personForm.address_line1"
                    type="text"
                    class="mt-1 input-field"
                ...
                <div>
                  ...
                  >
                  <input
                    v-model="personForm.address_line2"
                    type="text"
                    class="mt-1 input-field"
                ...
                <div class="md:col-span-2">
                  ...
                  >
                  <input
                    v-model="personForm.email_address"
                    type="email"
                    class="mt-1 input-field"
              ...
              <div class="flex items-center space-x-6">
                ...
                <label class="flex items-center">
                  <input
                    v-model="personForm.person_of_interest"
                    type="checkbox"
                    class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                  />
                  <span class="ml-2 text-sm text-gray-700"
                    >Person of Interest</span
                  >
                </label>
              </div>
...
const saveChanges = async () => {
  ...
  try {
    ...
    const result = await powerSyncStore.updateExistingPlot(
      effectivePlot.value.id,
      updatedPlotData,
    );
    ...
    console.log(
      "Plot details saved successfully, sync will happen automatically",
    );
    ...
    if (error.message === "Plot not found") {
      alert(
        "This plot was recently created and is not yet available for editing. Please wait a moment and try again, or refresh the page.",
      );
    } else {
  ...
  try {
    ...
    try {
      await powerSyncStore.powerSync?.execute(
        "DELETE FROM plot_images WHERE plot_id = ?",
        [effectivePlot.value.id],
      );
...
const capturePhoto = async () => {
  try {
    ...
    alert(
      "Failed to capture photo. Please ensure camera permissions are granted.",
    );
  }
};
...
const deleteImage = async (imageId) => {
  if (confirm("Are you sure you want to delete this photo?")) {
    try {
      await powerSyncStore.powerSync?.execute(
        "DELETE FROM plot_images WHERE id = ?",
        [imageId],
      );
    } catch (err) {
    ...
    try {
      ...
      if (age === 0) {
        const daysDiff = Math.floor(
          (deathDate - birthDate) / (1000 * 60 * 60 * 24),
        );
        if (daysDiff < 30) {
...
watch(
  ...
  async (newPlot) => {
    ...
    try {
      ...
      // Detect format
      const isLatLngFormat = coordinates.some(
        (coord) => Math.abs(coord[0]) <= 180 && Math.abs(coord[1]) <= 90,
      );
      let mapCoordinates = isLatLngFormat
      ...
      const tileSource = await createBestTileSource(
      ...
      setTimeout(() => {
        try {
          miniMap.getView().fit(plotFeature.getGeometry().getExtent(), {
            padding: [20, 20, 20, 20],
          });
        } catch (error) {
...
const deletePerson = async (person) => {
  if (
    confirm(
      `Are you sure you want to delete ${person.full_name} from this plot?`,
    )
  ) {
    try {
      await personsStore.deletePerson(person.id);
