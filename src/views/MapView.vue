<template>
  <div class="h-full flex flex-col min-h-0">
    ...
    <div class="flex-1 relative min-h-0 map-container h-full">
      <MapComponent
        ...
        :max-zoom="settingsStore.maxZoom"
        :min-zoom="settingsStore.minZoom"
        :extent="mapExtent"
        :restrict-extent="isExtentConfigured"
        :location="currentLocation"
        view-name="MainMap"
        tile-source-name="MainMap"
        :show-zoom="true"
      ...
      <div
        v-if="drawingMode"
        class="absolute top-4 left-4 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg z-20"
      >
        <p class="text-sm">
          Click to draw plot area. Plot will be created as 8×4ft rectangle
          centered on your drawing.
        </p>
      </div>
      ...
      >
        ...
        >
          <MapEdit
            :is-visible="true"
            title="Edit Plot"
            instructions="Adjust the plot position, size, and rotation on the map"
            save-button-text="Save Changes"
            :existing-plot="selectedPlot"
            :max-zoom="settingsStore.maxZoom"
            :min-zoom="settingsStore.minZoom"
            :zoom="settingsStore.defaultZoom"
            @save="handlePlotEditSave"
            @cancel="closeMapEdit"
            @close="closeMapEdit"
          />
        </div>
      ...
      <div
        v-if="selectedPlot"
        class="absolute top-4 right-4 bg-white rounded-lg shadow-lg p-4 max-w-sm z-20 max-h-[calc(100vh-8rem)] overflow-y-auto"
      >
        <h3 class="font-medium text-gray-900 mb-2">
          Plot {{ selectedPlot.section }}-{{ selectedPlot.row }}-{{
            selectedPlot.number
          }}
        </h3>
        <p class="text-sm text-gray-600 mb-2">{{ selectedPlot.status }}</p>
        <p class="text-sm text-gray-600 mb-3">
          {{ selectedPlotPersonsNames || "No deceased persons" }}
        </p>
        ...
        <div class="flex space-x-2">
          ...
          <button
            ...
            :title="
              editingMode
                ? 'Finish Editing & Save'
                : 'Start Editing (Scale, Rotate, Move)'
            "
          >
            <svg
              v-if="!editingMode"
          ...
          <button
            @click="mapStore.clearSelectedPlot"
  ...
  <PlotCreationWizard
    :is-visible="showPlotCreationWizard"
    :initial-location="persistentContextLocation"
...
const selectedPlotPersonsNames = computed(() =>
  mapStore.selectedPlotPersons.map((person) => person.full_name).join(", "),
);
...
onMounted(() => {
  plotSelectedHandler = async (event) => {
    ...
    if (
      plotId &&
      selectedPlot.value?.id === plotId &&
      plotImagesQuery.refetch
    ) {
    ...
    if (selectedPlot.value) {
      ...
      for (const layer of layers) {
        if (layer.get("name") === "plots") {
          ...
          plotFeature = features.find(
            (f) => f.get("plot")?.id === selectedPlot.value.id,
          );
      ...
      if (plotFeature) {
        ...
        await mapStore.saveInPlacePlotCoordinates(
          latLngCoordinates,
...
watch(
  ...
  async ([newPlots, map]) => {
    ...
    const existingPlotIds = new Set(
      existingFeatures.map((f) => f.get("plot")?.id).filter(Boolean),
    );

    // Find plots to add (in plots array but not on map)
    const plotsToAdd = (newPlots || []).filter(
      (plot) => !existingPlotIds.has(plot.id),
    );
    ...
    if (plotsToAdd.length > 0) {
      const plotPromises = plotsToAdd.map((plot) =>
        mapStore.addPlotMarkerOptimized(plot),
      );
