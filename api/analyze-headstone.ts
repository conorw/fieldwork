...
async function compressImage(
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Image size: ${imageSizeKB.toFixed(2)}KB - resizing recommended but sharp not available`,
      );
    ...
    console.log(
      `Image compressed: ${imageSizeKB.toFixed(2)}KB -> ${compressedSizeKB.toFixed(2)}KB (${((1 - compressedSizeKB / imageSizeKB) * 100).toFixed(1)}% reduction)`,
    );
...
async function analyzeHeadstoneWithOpenAI(
  imageData: string,
  mimeType: string,
): Promise<{ persons: HeadstonePerson[]; full_text_transcription: string }> {
  ...
  const structuredOutputSchema = {
    ...
    properties: {
      full_text_transcription: {
        type: "string",
        description:
          "Complete transcription of all text visible on the headstone, preserving line breaks and formatting",
      },
      persons: {
        type: "array",
        ...
        items: {
          ...
          properties: {
            ...
            age_at_death: {
              type: "integer",
              description:
                "Age at death. If not present, calculate from date_of_birth and date_of_death. Consider infant deaths as decimal. Do not round.",
            },
            time_of_death: {
              type: "string",
              description: "Time of death (HH:MM)",
            },
            birth_city: { type: "string", description: "Birth city" },
            birth_sub_country: {
              type: "string",
              description: "Birth province/state",
            },
            ...
            deceased: {
              type: "boolean",
              description: "Deceased (always true)",
            },
            person_of_interest: {
              type: "boolean",
...
Provide complete transcription of ALL text in full_text_transcription field, preserving line breaks.`;
  ...
  try {
    ...
    if (response.usage) {
      console.log(
        `OpenAI Token Usage: ${response.usage.prompt_tokens} prompt + ${response.usage.completion_tokens} completion = ${response.usage.total_tokens} total`,
      );
    }

    // Convert OpenAI response to our HeadstonePerson format, handling missing/empty values
    const persons: HeadstonePerson[] = analysisData.persons.map(
      (person: any) => {
        ...
        return {
          ...
          age_at_death:
            person.age_at_death !== undefined && person.age_at_death !== ""
              ? person.age_at_death
              : null,
          ...
          person_of_interest:
            person.person_of_interest === true ||
            person.person_of_interest === "true",
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const analysisResult = await analyzeHeadstoneWithOpenAI(
      compressedImageData,
      mimeType,
    );
    ...
    console.log(
      `Analysis complete: Found ${analysisResult.persons.length} person(s)`,
    );
    console.log(
      `Full text transcription length: ${analysisResult.full_text_transcription.length} characters`,
    );
